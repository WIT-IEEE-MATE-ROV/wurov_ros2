...
def callback(data):
    """
    Is this the servo we've been told to deal with? If so, deal with it here.
    """
    if data.executor.lower() == Name.lower():
        msg = arbitrary_pca_commands()

        msg.set_thruster = False
        msg.set_channel_pwm = True
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser('Runs a servo of a specified name.')
    ...
    # Creates a node name like "servo_claw_11"
    rospy.init_node('servo_' + Name + '_ch'+str(Channel), anonymous=False)
    rospy.spin()
